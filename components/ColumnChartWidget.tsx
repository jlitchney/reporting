...
export default function ColumnChartWidget({ data, config, accent, onUpdate, onRemove, canRemove, showDragHandle, onDuplicate, readOnly }: Props) {
  ...
  return (
    ...
    >
      <div className="px-4 pb-4 pt-2">
        {chartData.length === 0 ? (
          <div className="flex h-48 items-center justify-center text-sm text-slate-400">No data for the selected criteria</div>
        ) : (
          <ResponsiveContainer width="100%" height={chartHeight}>
            <BarChart layout="vertical" data={chartData} margin={{ top: 4, right: 56, left: 0, bottom: 4 }}>
              ...
              />
              <Tooltip
                formatter={(v: any) => [Number(v).toLocaleString(), 'Candidates']}
                contentStyle={{ fontSize: 12, borderRadius: 8, border: '1px solid #e2e8f0' }}
              />
              <Bar dataKey="count" fill={accent} radius={[0, 4, 4, 0]} maxBarSize={28}>
                <LabelList dataKey="count" position="right" style={{ fontSize: 10, fill: '#64748b' }} formatter={(v: unknown) => (Number(v) > 0 ? Number(v) : '')} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        )}
