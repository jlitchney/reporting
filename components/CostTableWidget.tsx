...
import { computeCostMetrics } from '@/lib/dataProcessor';
          ...
          <table className="w-full border-collapse text-xs">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                <th className="sticky left-0 bg-slate-50 px-2 py-2 text-left font-semibold text-slate-600">
                  Week
                </th>
                {sources.map((src) => (
                  <th key={src.label} colSpan={showApps && showCandidates ? 3 : 2} className="px-2 py-2 text-center font-semibold text-slate-600 border-l border-slate-200">
                    {src.tag}
                  </th>
                ))}
                <th colSpan={showApps && showCandidates ? 4 : 2} className="px-2 py-2 text-center font-semibold text-slate-600 border-l border-slate-200">
                  Total
                </th>
              </tr>
              <tr className="border-b border-slate-200 bg-slate-50/50">
                ...
                {sources.flatMap((src) => [
                  showApps && (
                    <th key={`${src.label}-apps`} className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">Apps</th>
                  ),
                  showCandidates && (
                    <th key={`${src.label}-cands`} className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">Cands</th>
                  ),
                  <th key={`${src.label}-spend`} className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">Spend</th>,
                  showApps && (
                    <th key={`${src.label}-cpa`} className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">$/App</th>
                  ),
                  showCandidates && (
                    <th key={`${src.label}-cpc`} className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">$/Cand</th>
                  ),
                ].filter(Boolean))}
                {showApps && (
                  <th className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-200">Apps</th>
                )}
                {showCandidates && (
                  <th className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">Cands</th>
                )}
                <th className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">Spend</th>
                {showApps && (
                  <th className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">$/App</th>
                )}
                {showCandidates && (
                  <th className="px-2 py-1 text-right font-medium text-slate-500 border-l border-slate-100">$/Cand</th>
                )}
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => (
                <tr key={row.weekLabel} className={`border-b border-slate-100 ${i % 2 === 1 ? 'bg-slate-50/40' : ''}`}>
                  <td className="sticky left-0 bg-white px-2 py-1.5 font-medium text-slate-700">
                    {row.weekLabel}
                  </td>
                  {row.sources.flatMap((src) => [
                    showApps && (
                      <td key={`${src.tagLabel}-apps`} className="px-2 py-1.5 text-right text-slate-700 border-l border-slate-100">
                        {src.applications}
                      </td>
                    ),
                    showCandidates && (
                      <td key={`${src.tagLabel}-cands`} className="px-2 py-1.5 text-right text-slate-700 border-l border-slate-100">
                        {src.candidates}
                      </td>
                    ),
                    <td key={`${src.tagLabel}-spend`} className="px-2 py-1.5 text-right text-slate-700 border-l border-slate-100">
                      {src.spend > 0 ? fmt(src.spend) : '—'}
                    </td>,
                    showApps && (
                      <td key={`${src.tagLabel}-cpa`} className="px-2 py-1.5 text-right text-slate-700 border-l border-slate-100">
                        {src.costPerApp != null ? fmt(src.costPerApp) : '—'}
                      </td>
                    ),
                    showCandidates && (
                      <td key={`${src.tagLabel}-cpc`} className="px-2 py-1.5 text-right text-slate-700 border-l border-slate-100">
                        {src.costPerCandidate != null ? fmt(src.costPerCandidate) : '—'}
                      </td>
                    ),
                  ].filter(Boolean))}
                  {showApps && (
                    <td className="px-2 py-1.5 text-right font-medium text-slate-700 border-l border-slate-200">
                      {row.totals.applications}
                    </td>
                  )}
                  {showCandidates && (
                    <td className="px-2 py-1.5 text-right font-medium text-slate-700 border-l border-slate-100">
                      {row.totals.candidates}
                    </td>
                  )}
                  <td className="px-2 py-1.5 text-right font-medium text-slate-700 border-l border-slate-100">
                  ...
                  {showApps && (
                    <td className="px-2 py-1.5 text-right font-medium text-slate-700 border-l border-slate-100">
                      {row.totals.costPerApp != null ? fmt(row.totals.costPerApp) : '—'}
                    </td>
                  )}
                  {showCandidates && (
                    <td className="px-2 py-1.5 text-right font-medium text-slate-700 border-l border-slate-100">
                      {row.totals.costPerCandidate != null ? fmt(row.totals.costPerCandidate) : '—'}
                    </td>
                  )}
                </tr>
              ))}
